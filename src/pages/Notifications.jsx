...
const Notifications = () => {
  ...
  const handleGetNotifications = async () => {
    ...
    const notis = resp.data
      ...
      .map((notification) => {
        const { __v, ...rest } = notification; // Exclude _id and __v
        return rest;
      });
                      ...
                      <tr key={idx} className=''>
                        <td className='py-3 '>
                          <div className='flex items-center gap-1 px-4 py-1'>
                            ...
                            <p className='text-[14px] font-inter text-white88 leading-[20px]'>{notification?.data}</p>
                        ...
                        <td className='flex justify-between w-[90px]'>
                          <CircleCheck onClick={() => {updateNotification(notification._id, 'read')}} className={`${notification.isRead ? "text-primaryYellow/20 cursor-not-allowed" : "text-primaryYellow/70 cursor-pointer"}`} size={30} />
                          <Trash2 onClick={() => {updateNotification(notification._id, 'delete')}} className='text-primaryYellow/70 cursor-pointer' size={28} />
                        </td>
